...
class PRAUCMetric(PluginMetric[float]):
    """
    PluginMetric to compute PR-AUC for the attack class (class 1).
    """

    def __init__(self, name="pr_auc_attack"):
        """
    ...
    def result(self ) -> MetricValue:
        """
        Compute and return the PR-AUC as a MetricValue.
        """
        ...
        # Compute PR-AUC for class 1 (attack)
        if torch.isnan(torch.tensor(self.y_prob)).any() :
            print(self.y_prob,len(self.y_prob) )

        precision, recall, _ = precision_recall_curve(self.y_true, self.y_prob, pos_label=1)
        ...
        return MetricValue(
            origin=self,
            name=self.name,
            value=round(pr_auc, 3),
            x_plot=0
        )

    def after_training_epoch(self, strategy) -> None:
        """
        Compute and return MetricValue after each training epoch.
        """
        # metric_value = self.result()
        # self.reset()
        # return [metric_value]

    def after_eval_exp(self, strategy) -> None:
        """
        Compute and return MetricValue after each evaluation experience.
        """
        metric_value = self.result()
        self.reset()
        return [metric_value]
    ...
    def before_training_epoch(self, strategy) -> None:
        """
        Reset the metric before each training epoch.
        """
        # self.reset()

    def before_eval_exp(self, strategy) -> None:
        """
        Reset the metric before each evaluation experience.
        """
        # self.reset()

    def after_training_iteration(self, strategy) -> None:
        """
        Update the metric with the current batch predictions.
        """
        # self._update_metric(strategy)

    def after_eval_iteration(self, strategy) -> None:
        """
        Update the metric with the current batch predictions during evaluation.
        """
        self._update_metric(strategy)

    def _update_metric(self, strategy):
        """
        Helper method to update true labels and predicted probabilities.
        """
        outputs = strategy.mb_output
        softmax = torch.nn.Softmax(dim=1)
        probabilities = softmax(outputs)

        self.y_true.extend(strategy.mb_y.cpu().numpy())
        self.y_prob.extend(probabilities[:, 1].cpu().detach().numpy())
