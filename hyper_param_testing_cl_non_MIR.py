...
if __name__ == "__main__":
    ...
    parser.add_argument('--cl_method',type=str,metavar='S',default="MIR",help='Method of CL : MIR OR CBRS')
    ...
    for seed_value in seed_list:
        ...
        hyperparameter_grid = {
    'learning_rate': [0.1, 0.01,0.001,1e-4,1e-5,1e-6,1e-7],
    'weight_decay': [0.1, 0.01,0.001,1e-4,1e-5,1e-6,1e-7],
    ...
    for cl_method in methods:
        log_dir = os.path.join(curr_dir, f"hyper_parameter_test_results/{cl_method}")
        log_file_path = os.path.join(log_dir, f"experiment_results_{dataset}_aut_test.log")
                ...
                for lr in hyperparameter_grid['learning_rate']:
                    for w_d in hyperparameter_grid['weight_decay']:
                        ...
                        for seed_value in seed_list:
                            ...
                            log_file.write(f"Seed: {seed_value}, Result: {pr_auc_outlier}, Time:{time.time()-start_time}\n")
