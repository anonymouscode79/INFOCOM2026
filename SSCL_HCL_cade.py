...
from baselines.active_learning.train import evaluate_model, train_classifier, train_encoder,init_weights
from baselines.continual_learning.model_loader import Model_class
...
def run_tsne(data, labels, dataset_name, label_ratio, perplexity, learning_rate, n_iter, random_state, save_dir):
    """
    Run t-SNE for given parameters and save the plot in PNG, PDF, and EPS formats.
    """
    tsne = TSNE(
        n_components=2,
    ...
    # Base file name with hyperparameters
    base_file_name = f"tsne_plot_perp{perplexity}_lr{learning_rate}_iter{n_iter}_rs{random_state}"
    png_path = os.path.join(save_dir, f"{base_file_name}.png")
    pdf_path = os.path.join(save_dir, f"{base_file_name}.pdf")
    eps_path = os.path.join(save_dir, f"{base_file_name}.eps")

    # Plot the results
    plt.figure(figsize=(10, 7))
    unique_labels = np.unique(labels)
    label_names = ["Benign" if label == 0 else "Attack" for label in unique_labels]
    
    for label, name in zip(unique_labels, label_names):
        subset = tsne_results[labels == label]
        plt.scatter(subset[:, 0], subset[:, 1], label=name, s=50, alpha=0.7)

    plt.title(f"t-SNE (Perplexity={perplexity}, Iter={n_iter})")
    plt.xlabel("t-SNE Dimension 1")
    plt.ylabel("t-SNE Dimension 2")
    plt.grid(True)
    plt.legend(title="Class", loc='best')  # Add legend
    plt.tight_layout()
    ...
    print(f"t-SNE plot saved at: {png_path}, {pdf_path}, {eps_path}")

def tsne_visualization_parallel(data, labels, dataset_name, label_ratio, perplexities, learning_rates, iterations, random_state=42, n_jobs=-1,time_frame="after_seen"):
    """
    Perform parallel t-SNE visualization for multiple hyperparameter combinations.

    Parameters:
        data (numpy.ndarray): High-dimensional data (n_samples x n_features).
        labels (numpy.ndarray or list): Binary labels for "Attack" (1) and "Benign" (0).
        dataset_name (str): Name of the dataset.
        label_ratio (str): Ratio of labels as a string (e.g., "70_30").
        perplexities (list): List of perplexity values to try.
        learning_rates (list): List of learning rate values to try.
        iterations (list): List of iteration values to try.
        random_state (int): Random seed for reproducibility. Default is 42.
        n_jobs (int): Number of parallel jobs (-1 for all CPUs).

    Returns:
        None: Saves the t-SNE plots in the specified directory.
    """
    # Normalize the data
    ...
    save_dir = f"tsne_plots/{dataset_name}/{label_ratio}/{time_frame}"
    os.makedirs(save_dir, exist_ok=True)

    # Run t-SNE for all parameter combinations in parallel
    Parallel(n_jobs=n_jobs)(
        delayed(run_tsne)(
            data, labels, dataset_name, label_ratio,
            perplexity, learning_rate, n_iter, random_state, save_dir
        )
...
def train_baseline_hcl_good(supervised = True,threshold = 0.8,label_ratio=0.2,plot_tsne=False):
    global input_shape
    # Step 1 : Load Tasks
    train_order = task_order[:training_cutoff]
    test_order = task_order[training_cutoff:]
    X_for_tsne_plot = None
    y_for_tsne_plot = None
    # Step 2 : Select Model
    if type_selector =="pseudo-loss" :
        enc_dims = [input_shape,512,384,256,128]
        mlp_dims = [128,100,100,2]
    ...
    for task_id , task in enumerate(train_order):
        ...
        if type_selector == "cade":
            ...
            if label_ratio != 1.0:
                ...
                selected_attack_indices = np.random.choice(attack_indices,int(label_ratio * len(attack_indices)))
                selected_benign_indices = np.random.choice(benign_indices,int(label_ratio * len(benign_indices)))
                total_sample_indices = np.concatenate([selected_attack_indices,selected_benign_indices])
                X, y, y_bin = X[total_sample_indices],y[total_sample_indices],y_bin[total_sample_indices]
            total_X_per_task = np.concatenate([X,X_test,X_val])
            total_y_bin_per_task = np.concatenate([y_bin,y_test,y_val])
            print(f"Shape of Total is :{total_X_per_task.shape}")
            if task_id ==0:
                X_for_tsne_plot = total_X_per_task
                y_for_tsne_plot = total_y_bin_per_task
            else:
                X_for_tsne_plot = np.concatenate([X_for_tsne_plot, total_X_per_task], axis=0)
                y_for_tsne_plot = np.concatenate([y_for_tsne_plot, total_y_bin_per_task], axis=0)

            train_encoder(encoder,X_train=X,y_train=y,y_train_binary=y_bin,optimizer=optimizer_enc,total_epochs=50,model_path="models/hcl",device=device,
                      batch_size=batch_size,valid_loader=valid_loader,family_info=family_info,type_selector=type_selector,lambda_=100)

    ### Plotting the TSNE for visualization 

    if plot_tsne :
        print(f"Shape while tsne Visualization:")
        X_for_tsne_plot_encoded = encoder.encode(torch.from_numpy(X_for_tsne_plot).float().to(device)).detach().cpu().numpy()
        print(f"Shape while tsne Visualization:{X_for_tsne_plot_encoded.shape}")
        perplexities = [30, 50, 100]
        tsne_learning_rates = [200, 500, 1000, 2000]
        iterations = [1000, 2000, 3000]
        tsne_visualization_parallel(X_for_tsne_plot_encoded, y_for_tsne_plot, ds, label_ratio=label_ratio, perplexities=perplexities,learning_rates=tsne_learning_rates,iterations=iterations, random_state=seed)
    ### For the Unseen Tasks
    ...
    for task_id, task in enumerate(test_order):
        ...
        if plot_tsne:
            X_for_tsne_plot = np.concatenate([X_for_tsne_plot, X_test], axis=0)
            y_for_tsne_plot = np.concatenate([y_for_tsne_plot, y_test_bin], axis=0)
        # Initial evaluation
        if type_selector == "pseudo-loss":
            print("Evaluating before training...")
            metrics_before = evaluate_model(encoder, valid_loader, device, None)
        ...
        if type_selector == "pseudo-loss":
            ...
            sample_indices, sample_scores = selector.select_samples(X_train, y_train, X_test, analyst_label)
    ...
    if plot_tsne:
        ...
        tsne_learning_rates = [200, 500, 1000, 2000]
        iterations = [1000, 2000, 3000]
        tsne_visualization_parallel(X_for_tsne_plot_encoded, y_for_tsne_plot, ds, label_ratio=label_ratio, perplexities=perplexities,learning_rates=tsne_learning_rates,iterations=iterations, random_state=seed,time_frame="after_whole")
def train_baseline_hcl_no_sample_selection(supervised = True,threshold = 0.8):
    global input_shape
    # Step 1 : Load Tasks
    train_order = task_order[:training_cutoff]
    test_order = task_order[training_cutoff:]
            ...
            with torch.no_grad():
                for m in models:
                    if model is not None:  # Skip models that are None
                        if type_selector == "cade":
                            data_enc = model.encode(data.to(device))
                            outputs = classifier.predict_proba(data_enc)
                        elif type_selector =="pseudo-loss":
                            outputs = model.predict_proba(data.to(device))  # Get predictions
                    else:
                        outputs = model(data.to(device))  # Get predictions
...
def start_execution(dataset_name,l_rate,w_decay):
    ...
    if is_lazy_training:
        ...
        # get_whole_test_set()
        train_baseline_hcl_good(label_ratio=labels_ratio,plot_tsne=False)
        # train_baseline_hcl_no_sample_selection()
        # train_CL()
        # taskwise_lazytrain()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--analyst_label', type=int, metavar='S',default=100,help='no of samples labelled by Analyst')
    ...
    parser.add_argument('--is_supervised',type=str,metavar='S',default="True",help='Supervised or semi-supervised')
    args = parser.parse_args()
    set_seed(args.seed)
    get_gpu(args.gpu)
    print("seed is",args.seed)
    global analyst_label,labels_ratio,no_of_rand_samples,l_rate,w_decay,batch_minority_allocation,b_m,alpha,lab_samp_in_mem_ratio,bool_gpm,mem_strat,temp_filename,auc_result,seed,bool_closs,mlps,training_cutoff, epochs, ds, beta,cos_dist_ip, mode_value
    ...
    family_info=str2bool(args.family_info)
    is_supervised=str2bool(args.is_supervised)
    analyst_label=args.analyst_label
